...
export async function checkOnce() {
  ...
  if (dispoResidences.length > 0) {
      ...
      await notify(
        isEligible
          ? messageDispo +
              ...
              `\n   • Il choisit le meilleur selon tes préférences (${escapeHtml(config.preferredTypes.join(' > '))}, sans colocation),` +
              ` le réserve, PUIS il relit ton compte pour vérifier que c'est vraiment pris.` +
    ...
    // La tentative HTTP est suivie d'une RELECTURE du compte, et au besoin d'un
    // rejeu dans un vrai navigateur : rien n'est annoncé sans vérification.
    if (config.mode === 'reserve') {
      ...
      try {
        await handleAvailability(html, cookieHeader, {
          ...
          residences: dispoResidences,
        });
      } catch (err) {
        console.error('[check] Traitement en échec:', err);
        await notify(
          `⚠️ Traitement de la dispo (${escapeHtml(contexte)}) en échec : <code>${escapeHtml(err.message)}</code>\n` +
